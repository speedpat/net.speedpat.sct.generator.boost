...
class Transitions {
	...
	def dispatch List<Transitions.TransitionTableEntry> transitions(ExecutionState state, List<Transitions.TransitionTableEntry> entries, Step step) {
		
	}
	
	def dispatch List<Transitions.TransitionTableEntry> transitions(ExecutionState state, List<Transitions.TransitionTableEntry> entries, If condition) {
	...
	def dispatch List<Transitions.TransitionTableEntry> transitions(ExecutionState state, List<Transitions.TransitionTableEntry> entries, Sequence seq) {
	...
	def dispatch Transitions.TransitionTableEntry transition(Transitions.TransitionTableEntry entry, OperationDefinition exec) {
		entry.action = exec;
		return entry;
	}
	
	def dispatch Transitions.TransitionTableEntry transition(Transitions.TransitionTableEntry entry, ElementReferenceExpression ref) {
		ref.transition(entry, ref.definition)
	}
	
	def dispatch Transitions.TransitionTableEntry transition(ElementReferenceExpression re, Transitions.TransitionTableEntry entry, Declaration target) {
		return entry;
	}
	
	def dispatch Transitions.TransitionTableEntry transition(ElementReferenceExpression ref, Transitions.TransitionTableEntry entry, Event event) {
